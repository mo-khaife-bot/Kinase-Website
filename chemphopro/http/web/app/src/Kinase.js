...
import KinaseDescription from "./KinaseDescription";
...
function Kinase() {
    ...
    const kinaseName = location.state.name;


    const [kinase, setKinase] = useState({});
    const [pdtData, setPdtData] = useState([]);
    const [plotlyData, setPlotlyData] = useState({});
    const [knowKinases, setKnowKinases] = useState([]);
    ...
    useEffect(() => {
        ...
        fetch(foldChangeUrl)
            ...
            .then((result) => {
                ...
                var phosphositeTreatmentsFoldChange = Object.keys(result).map((phosphosite, index) => {
                    return [phosphosite, { x: result[phosphosite].map(x => { return x["treatment"] }), y: result[phosphosite].map(x => { return Math.log(x["fold_change"] + 0.0005) }), type: 'bar' }]
                })
        ...
        // url to get the known kinases
        const knowKinasesUrl = "http://localhost:5000/knownkinases/" + kinaseName
        // get the known kinases
        fetch(knowKinasesUrl)
            .then((result) => result.json())
            .then((result) => {
                // setKnowKinases to trigger the update of Previously reported substrates table 
                setKnowKinases(result)
            })
    }, kinaseName);
    ...
    return <div className={classes.root}>
        <Grid container spacing={3} justifyContent="center">
            <Grid item xs={8}>
                <KinaseDescription kinase={kinase} />
            </Grid>
            <Grid item xs={8}>
                <MaterialTable
                    icons={tableIcons}
                    title="Putative downstream targets (PDTs)"
                    columns={[
            ...
            <Grid item xs={8}>
                <MaterialTable
                    icons={tableIcons}
                    title="Previously reported substrates"
                    columns={[
                        { title: 'Phosphosite', render: x => x["substrate_genesymbol"] + "(" + x["residue_type"] + x["residue_offset"] + ")" },
                        { title: 'Protein name', render: x => <a href={'https://www.uniprot.org/uniprot/' + x.substrate} target="_blank">{x["substrate_genesymbol"]}</a> },
                        { title: 'Residue', field: 'residue_type' },
                        { title: 'Residue Source', field: 'references' }
                    ]}
                    data={knowKinases}
                />
            </Grid>
